...
class YouTubeDownloader:
    def __init__(self, root):
        ...
        self.thumbnails = {}  # Store thumbnail images
        self.download_progress = {}  # Track progress for each video
    ...
    def setup_ui(self):
        ...
        ttk.Label(row1_frame, text="Format:", font=('Arial', 10, 'bold')).pack(side=tk.LEFT, padx=5)
        ...
        ttk.Label(row1_frame, text="Quality:", font=('Arial', 10, 'bold')).pack(side=tk.LEFT, padx=(20, 5))
        ...
        ttk.Checkbutton(row2_frame, text="Download subtitles", variable=self.subtitles_var).pack(side=tk.LEFT, padx=20)
        
        self.thumbnail_var = tk.BooleanVar()
        ttk.Checkbutton(row2_frame, text="Download thumbnail", variable=self.thumbnail_var).pack(side=tk.LEFT, padx=20)
        
        # NEW: Add track numbers checkbox
        self.track_numbers_var = tk.BooleanVar(value=True)
        ttk.Checkbutton(
            row2_frame, 
            text="🔢 Add track numbers to filename (01_, 02_, etc.)", 
            variable=self.track_numbers_var
        ...
        main_content.add(left_panel, weight=2)
        
        # Content Display Frame
        content_frame = ttk.LabelFrame(left_panel, text="📋 Available Content", padding=10)
        content_frame.pack(fill=tk.BOTH, expand=True)
        
        # Treeview for displaying videos
        columns = ("selected", "index", "title", "duration", "quality", "size", "progress")
        ...
        self.tree.heading("quality", text="Qualities")
        ...
        self.tree.bind("<Double-Button-1>", self.on_tree_double_click)  # Double-click to show thumbnail
        ...
        # Thumbnail frame
        thumb_frame = ttk.LabelFrame(right_panel, text="🖼️ Thumbnail", padding=10)
        thumb_frame.pack(fill=tk.BOTH, expand=True, pady=(0, 5))
        
        self.thumb_label = ttk.Label(thumb_frame, text="Double-click a video\nto view thumbnail", 
                                     justify=tk.CENTER, font=('Arial', 10))
        self.thumb_label.pack(expand=True, fill=tk.BOTH)
        
        # Video details frame
        details_frame = ttk.LabelFrame(right_panel, text="📝 Video Details", padding=10)
        details_frame.pack(fill=tk.X, pady=5)
        
        self.details_text = tk.Text(details_frame, height=8, width=30, wrap=tk.WORD)
    ...
    def check_all_dependencies(self):
        ...
        try:
            ...
            messagebox.showwarning(
                ...
                "PIL/Pillow is not installed. Thumbnail viewing will be disabled.\n"
                "To enable thumbnails, install: pip install Pillow"
            )
        ...
        try:
            ...
            if is_playlist:
                    ...
                    for line in result.stdout.strip().split('\n'):
                        if line:
                            try:
                                ...
                                # Fetch thumbnail URL
                                thumb_url = f"https://img.youtube.com/vi/{video_info.get('id')}/mqdefault.jpg"
                ...
                if result.returncode == 0 and result.stdout:
                    ...
                    # Get thumbnail URL
                    thumb_url = video_info.get('thumbnail', f"https://img.youtube.com/vi/{video_info.get('id')}/mqdefault.jpg")
                    ...
                    videos = [{
                        ...
                        'size': self.estimate_size(duration, '720p'),
    ...
    def _update_tree(self, videos):
        ...
        for i, video in enumerate(videos, 1):
            ...
            # Store video data
            self.playlist_videos[i-1] = video
    ...
    def on_tree_double_click(self, event):
        """Handle double-click to show thumbnail and details"""
        region = self.tree.identify_region(event.x, event.y)
        if region == "cell":
            item = self.tree.identify_row(event.y)
            if item:
                index = int(self.tree.item(item, "values")[1])
                self.show_video_details(index)
    
    def show_video_details(self, index):
        """Show video details and thumbnail"""
        if 1 <= index <= len(self.playlist_videos):
            ...
            details += f"Duration: {video['duration']}\n"
            if 'uploader' in video:
                details += f"Uploader: {video['uploader']}\n"
            if 'upload_date' in video and video['upload_date'] != 'Unknown':
            ...
            if self.pil_available and 'thumbnail' in video:
                self.load_thumbnail(video['thumbnail'])
    
    def load_thumbnail(self, url):
        """Load thumbnail from URL and display"""
        try:
            # Download thumbnail
            ...
            self.thumb_label.image = photo  # Keep a reference
            
        except Exception as e:
            self.thumb_label.config(image="", text="Failed to load thumbnail")
    ...
    def toggle_checkbox(self, item):
        ...
        # Update the tree item, preserving all values including progress
        self.tree.item(item, values=(
            new_checkbox,
            current_values[1],  # index
            current_values[2],  # title
            current_values[3],  # duration
            current_values[4],  # quality
            current_values[5],  # size
            current_values[6]   # progress
        ))
    ...
    def select_all(self):
        ...
        for i, item in enumerate(self.tree.get_children(), 1):
            ...
            self.tree.item(item, values=("☑", i, current[2], current[3], current[4], current[5], current[6]))
    ...
    def clear_all(self):
        ...
        self.thumb_label.config(image="", text="Double-click a video\nto view thumbnail")
        self.details_text.config(state=tk.NORMAL)
        self.details_text.delete(1.0, tk.END)
        self.details_text.config(state=tk.DISABLED)
    ...
    def update_video_progress(self, index, percent):
        """Update progress for a specific video in the tree"""
        item_id = f"video_{index}"
        if self.tree.exists(item_id):
            current = self.tree.item(item_id, "values")
            # Format progress display
            if percent < 0:
                progress_display = "Waiting..."
            elif percent >= 100:
                progress_display = "✅ Complete"
            else:
                progress_display = f"{percent:.1f}%"
            
            self.tree.item(item_id, values=(
                current[0], current[1], current[2], current[3], 
                current[4], current[5], progress_display
            ))
    ...
    def start_download(self):
        ...
        response = messagebox.askyesno(
            ...
            f"Add track numbers: {'Yes' if self.track_numbers_var.get() else 'No'}\n\n"
            f"Continue?"
        )
        ...
        # Reset progress for selected videos
        for index in self.selected_videos:
            self.update_video_progress(index, 0)
    ...
    def _download_thread(self, download_dir):
        """Thread function to download videos - with progress tracking"""
        try:
            self.root.after(0, self.progress_bar.start)
            ...
            # Sort selected videos by index to maintain order
            sorted_indices = sorted(self.selected_videos)
            
            for idx, index in enumerate(sorted_indices, 1):
                video = self.playlist_videos[index - 1]
                
                # Update status
                self.root.after(0, lambda v=video: self.current_download_var.set(f"📥 Downloading: {v['title']}"))
                self.root.after(0, lambda c=completed, t=total: self.progress_var.set(f"Progress: {c}/{t}"))
                self.root.after(0, lambda v=video, c=idx, t=total: self.status_var.set(f"📥 Downloading {c}/{t}: {v['title'][:30]}..."))
                
                # Build download command
                format_type = self.format_var.get()
                quality = self.quality_var.get()
                
                # Create filename with optional track number
                if self.track_numbers_var.get() and len(sorted_indices) > 1:
                    # Add track number with leading zero (01_, 02_, etc.)
                    track_num = f"{idx:02d}_"
                else:
                    track_num = ""
                ...
                cmd = [
                    ...
                    "--progress",
                    "--newline",
                    "-o", os.path.join(download_dir, f"{track_num}%(title)s.%(ext)s")
                ]
                ...
                if self.subtitles_var.get():
                    cmd.extend(["--write-subs", "--sub-lang", "en", "--embed-subs"])
                ...
                try:
                        ...
                        if output:
                            ...
                            if '%' in output:
                                ...
                                if match:
                                    ...
                                    self.root.after(0, lambda p=percent, i=index: self.update_video_progress(i, p))
                                    self.root.after(0, lambda p=percent: self.current_download_var.set(f"Downloading... {p:.1f}%"))
                    ...
                    if return_code == 0:
                        ...
                        self.root.after(0, lambda i=index: self.update_video_progress(i, 100))
                        self.root.after(0, lambda: self.status_var.set(f"✅ Completed: {video['title'][:30]}..."))
                    else:
                        failed.append(video['title'])
                        self.root.after(0, lambda i=index: self.update_video_progress(i, -1))  # -1 for failed
