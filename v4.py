    ...
    def _download_thread(self, download_dir):
        ...
        try:
            ...
            for index in sorted(self.selected_videos):
                ...
                if format_type == "mp3":
                    # For MP3 audio - use best audio and convert
                    cmd.extend([
                        "-f", "bestaudio/best",
                        "-x",  # Extract audio
                        "--audio-format", "mp3",
                    ...
                    # Best quality - let yt-dlp choose the best combination
                    cmd.extend([
                        "-f", "bestvideo+bestaudio/best",
                        "--merge-output-format", "mp4"
                    ])
                else:
                    # For video formats with specific quality
                    quality_map = {
                ...
                try:
                    ...
                    for line in process.stdout:
                        if '%' in line:  # Progress line
                            self.root.after(0, lambda l=line: self.current_download_var.set(l.strip()))
